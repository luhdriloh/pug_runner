...
public class LetterStream : MonoBehaviour
{
    ...
    private char[] _values = "abcdefghijklmnopqrstuvwxyz1234567890".ToCharArray();

    private float _currentTime;

	private void Start ()
    {
        _numLettersToFadeToBase = 4;
    ...
    {
        _fontSize = Random.Range(20, 60);
        _guiStyle.fontSize = _fontSize;
        _fallSpeed = Random.Range(.05f, .2f);
        _maxLength = Random.Range(15, 40);
        _lettersFromBackToStartFading = Random.Range(4, 12);
        ...
        for (int i = 0; i < toDisplay.Length; i++)
        {
            ...
            if (toDisplay.Length >= _maxLength)
            {
                ...
                if (indexFromBack >= 0)
                {
                    ...
                    colorInUse.a = 1 - ((1f / _numLettersToFadeToBase) * indexFromBack);
